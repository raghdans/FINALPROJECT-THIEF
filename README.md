# Thief Agent - Distributed Police-vs-Thief Project

This is the thief submission repository. Its companion police repository URL must
replace `REPLACE_WITH_POLICE_REPOSITORY_URL` before the official submission.

> **Student implementation:** both roles are configured to use the original
> adaptive belief-and-mobility strategies in `src/team_strategy/adaptive.py`.
> See `docs/OUR-STRATEGY.md` for the design and compliance notes. Before playing
> an official match, replace the sample group identities, members and repository
> URLs in both private `config/<role>/game.toml` files.

## Academic report

### 1. Dec-POMDP model

The chase is modelled as a decentralized partially observable Markov decision
process. The joint state contains both true positions, barriers, turn number and
the decaying scent field, but each peer observes only its own true state, received
scent values, public barriers and an untrusted natural-language hint. Actions are
orthogonal movement, hold, and police-only barrier placement. The transition model
is deterministic for legal physical actions while the observation model remains
uncertain. Rewards follow the binding capture/survival table and are aggregated per
group across the six-game series. The belief grid is the local sufficient statistic
used by the strategy instead of forbidden access to the opponent's true position.

### 2. FastMCP orchestration and reliability

Each peer is simultaneously a FastMCP server and client. It owns its state and
accepts sealed turn, capture, audit and optional control messages. The orchestrator
implements handshake, agreement hashing, alternating turns, deadlines, game-over
and audit states. Start order is irrelevant because connection attempts retry until
the opponent is available. The Gatekeeper limits rate and concurrency, queues
bursts, backs off after transient errors and applies deadlines. A watchdog converts
silence beyond the negotiated limit into a technical result instead of leaving the
series hanging.

### 3. Decision strategies

Both roles use the original deterministic implementation in
`src/team_strategy/adaptive.py`. The thief maximizes expected distance from the
entire police belief distribution, penalizes local probability mass, revisits,
edges and corners, and rewards future mobility. The police minimizes expected
distance, rewards probability mass at the next cell and places a barrier only when
belief confidence is high and the wall provably removes an escape edge without
blocking the best capture cell. The move path is pure Python and consumes zero LLM
tokens; optional language generation cannot alter an action.

### 4. Learning curves

Reinforcement learning was deliberately not used. The strategy is explainable,
deterministic and requires no offline training data, so an RL learning curve would
be misleading. Evaluation is performed through repeatable seeded matches, series
scores, capture/survival rate, move count, audit success and token consumption.

### 5. Visual evidence

The live view exposes only the legal local belief state:

![Live GUI and belief heatmap](docs/images/gui-annotated.png)

The replay evidence below was generated from an actual six-game localhost run. The
shown game re-verified every commit and found no failed step:

![Replay Verified OK](docs/images/replay-verified.png)

### 6. Companion repository

- Police repository: https://github.com/raghdans/FINALPROJECT
- Thief repository: https://github.com/raghdans/FINALPROJECT-THIEF

The two URLs must be replaced after repository creation and must appear in both
repositories, the course form and the final JSON reports.
...
> <!--CODE_VERSION_START-->**Code `v3.0.0`**<!--CODE_VERSION_END--> · <!--BOOK_VERSION_START-->based on the **guidelines book `v3.0.0`**<!--BOOK_VERSION_END--> — the full rules & guidelines PDF is bundled at [`docs/police_thief_p2p.pdf`](docs/police_thief_p2p.pdf).
>
