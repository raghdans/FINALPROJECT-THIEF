...
def progression(summary):
    ...
    role = summary["role"]
    total = max(1, len(summary["history"]))
    checkpoints = sorted({max(1, total // 3), max(1, 2 * total // 3), total})
    while len(checkpoints) < 3:
        checkpoints.insert(0, 1)
    captions = ["Early: belief still diffuse", "Mid: belief concentrating",
                "Late: a hot cell (best guess)"]
    for i, (step, cap) in enumerate(zip(checkpoints[-3:], captions, strict=True)):
...
def gui(summary):
    step = max(1, min(17, len(summary["history"])))
    m, pos, vis, bar = reconstruct(summary, step)
...
def replay_verified(summary):
    """Render submission evidence from the real local run's audit data."""
    step = max(1, len(summary["history"]))
    matrix, pos, visited, barriers = reconstruct(summary, step)
    side = GRID * CELL
    image = Image.new("RGB", (900, side + 150), "#f4f4f4")
    draw = ImageDraw.Draw(image)
    draw.text((20, 18), "Replay Viewer - cryptographic verification", fill=INK,
              font=font(18, True))
    draw_board(draw, 20, 58, matrix, pos, visited, barriers, summary["role"])
    audit = summary.get("audit", {})
    colour = "#1b7f3a" if audit.get("passed") else "#b00020"
    status = "VERIFIED OK" if audit.get("passed") else "TAMPERED"
    x = side + 65
    draw.rounded_rectangle([x, 70, 850, 145], radius=10, fill=colour)
    draw.text(((x + 850) // 2, 107), status, fill="white", font=font(24, True), anchor="mm")
    rows = [
        ("Result", summary.get("result", "-")),
        ("Winner", summary.get("winner", "-")),
        ("Steps", str(summary.get("steps", "-"))),
        ("Verified commits", str(audit.get("verified_steps", 0))),
        ("Failed commits", str(len(audit.get("failed_steps", [])))),
        ("LLM tokens", str(summary.get("tokens_total", 0))),
    ]
    y = 180
    for label, value in rows:
        draw.text((x, y), f"{label}:", fill=INK, font=font(13, True))
        draw.text((x + 165, y), value, fill="#333333", font=font(13))
        y += 40
    draw.text((20, side + 92), "Generated from the saved local match log; all commits re-verified.",
              fill="#555555", font=font(12))
    image.save(OUT / "replay-verified.png")
